...
const Skills = () => {
  ...
  const getSkillStatus = (skill: any): 'locked' | 'available' | 'unlocked' | 'maxed' => {
      ...
      <div className="flex gap-1.5 sm:gap-2 mb-6 sm:mb-8 overflow-x-auto pb-2 -mx-4 px-4 sm:mx-0 sm:px-0">
        {skillTrees.map((tree: any) => (
          <button
            ...
            className={cn(
              "flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-2 rounded-lg font-medium transition-all duration-200 whitespace-nowrap text-xs sm:text-sm flex-shrink-0",
          ...
          >
            <span className="text-base sm:text-lg">{tree.emoji}</span><span>{tree.name}</span>
          </button>
        ))}
      </div>
              ...
              return (
                <motion.div
                  ...
                  className={cn(
                    ...
                    status === 'locked' && "border-border bg-card/50 opacity-60"
                  )}
                ...
                >
                  <div className="absolute -top-2 -right-2">
                    ...
                    {status === 'locked' && <div className="h-5 w-5 sm:h-6 sm:w-6 rounded-full bg-background-tertiary flex items-center justify-center"><Lock className="h-3 w-3 sm:h-3.5 sm:w-3.5 text-foreground-muted" /></div>}
                  </div>
